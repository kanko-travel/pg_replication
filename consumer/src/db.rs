...
impl<Key: ToString + Clone> Lock<Key> {
    ...
    pub async fn acquire(self, tx: &mut Transaction<'_, Postgres>) -> Result<(), ConsumerError> {
        sqlx::query("SELECT pg_advisory_xact_lock($1)")
            ...
            .map_err(|err| {
                ...
                ConsumerError::Recoverable(anyhow!("db error: {}", err))
            })?;
...
impl<Key: ToString + Clone> Lock<Key> {
    fn key(&self) -> Result<i64, ConsumerError> {
